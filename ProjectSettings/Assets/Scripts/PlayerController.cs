...
public class PlayerController : MonoBehaviour {
  ...
  //  private CapsuleCollider2D playerCollider;
    ...
    private void OnTriggerEnter2D(Collider2D other) {
        ...
        if(isShieldActive && (other.gameObject.tag == "Enemy" || other.gameObject.tag == "Asteroid" || other.gameObject.tag == "EnemyBullet" || other.gameObject.tag == "EnemyLaser")) {
            Instantiate(hitSound);
            isShieldActive = false;
            shield.SetActive(false);
        } else if(timer >= invicibilityTime && (other.gameObject.tag == "Enemy" || other.gameObject.tag == "Asteroid" || other.gameObject.tag == "EnemyBullet" || other.gameObject.tag == "EnemyLaser")) {
            Damage();
    ...
    private void OnTriggerStay2D(Collider2D other) {
        ...
        if(timer >= invicibilityTime && other.gameObject.tag == "Boss") {
            Damage();
            if(currentHealth.Equals(2)) { thirdHeart.gameObject.SetActive(false); } else if(currentHealth.Equals(1)) { secondHeart.gameObject.SetActive(false); } else {
                firstHeart.gameObject.SetActive(false);
                PlayExplosion();
                Instantiate(deathSound);
                gameObject.SetActive(false);
